...
if(isset($_GET['license']) && !empty(trim($_GET['license']))){
    // Set Text to Be Printed On Image
    $license_text = formatLicensePlate($_GET['license']);

    // Create Image From Existing File
    $png_image = imagecreatefrompng('dutch_license_plate.png');

    // Allocate A Color For The Text
    $white = imagecolorallocate($png_image, 0, 0, 0);
    // Set Path to Font File
    $font_path = 'Kenteken.ttf';
    // Print Text On Image
    imagettftext($png_image, 133, 0, 250, 220, $white, $font_path, $license_text);
